...
from server_src.MyConstants import DB_DATABASE, DEBUG
from server_src.DatabaseIO import db, Student
app = flask.Flask(__name__)
app.config['SECRET_KEY'] = "r`9[M-AtuO"
read_from_config_file("/etc/webrtc-Tony.conf")

DB_URL = "mariadb://{username}:{password}@{host}:{port}/{db}?charset=gbk".format(
    username=config_info['database']['user'], 
